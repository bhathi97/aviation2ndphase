...
Module ItemsModule
    ...
    'add all members to combo boxes
    Public Sub loadMoreCMToComboBox(connsql As SqlConnection, cb As ComboBox, group As String)
        Try

            Dim cmd As New SqlCommand("SELECT * FROM CREWMEMBERS_MASTER_TABLE where [GROUP] <> @gr and POSITION = 'CREWMEN'", connsql)
    ...
    'add all members to combo boxes
    Public Sub loadMoreRICToComboBox(connsql As SqlConnection, cb As ComboBox, group As String)
        Try

            Dim cmd As New SqlCommand("SELECT * FROM CREWMEMBERS_MASTER_TABLE where [GROUP] <> @gr and POSITION = 'RIC'", connsql)
            cmd.Parameters.AddWithValue("@gr", group)
            connsql.Open()
    ...
    'add RIC to the datagridview
    Public Sub loadMoreRICToDGV(cb As DataGridViewComboBoxColumn, lb As ListBox)
        Try
    ...
    'add RIC to the datagridview
    Public Sub loadMoreCMToDGV(cb As DataGridViewComboBoxColumn, lb As ListBox)
        Try
